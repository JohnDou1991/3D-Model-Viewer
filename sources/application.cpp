...
void Application::LoadLightSources()
{
   ...
   m_lights.emplace_back( std::make_unique<object::light::Directional>(color, attenuation, glm::vec3(3)) );
   m_lights.emplace_back( std::make_unique<object::light::Spot>(color, attenuation, m_camera->Position(), m_camera->Front(), 12.5f, 17.5f));
...
void Application::AddModels()
{
   ...
   {
      auto& program = AddProgram(m_shaders.object, model.second);
      program.LoadObject(model.first);
      glm::vec3 position(0,0,0);
      program.LoadTransformation([this, position](GLuint program)
      {
         ...
         glUniform3fv(glGetUniformLocation(program, "material.color.ambient"), 1, glm::value_ptr(glm::vec3(0.2, 0.2, 0.2)));
         glUniform3fv(glGetUniformLocation(program, "material.color.diffuse"), 1, glm::value_ptr(glm::vec3(0.2, 0.2, 0.2)));
         glUniform3fv(glGetUniformLocation(program, "material.color.specular"), 1, glm::value_ptr(glm::vec3(0.9, 0.9, 0.9)));

         glUniform1f(glGetUniformLocation(program, "material.shininess"), 2.0);
...
void Application::AddLights()
{
   for (auto& source : m_sources)
   {
      auto& program = AddProgram(m_shaders.light, {});
      program.LoadObject(m_models.front().first);

      program.LoadTransformation([this, &source](GLuint program)
      {
         glm::mat4 model = glm::mat4(1);
...
void Application::AddBackpack()
{
   auto& program = AddProgram(m_shaders.object, {});
   program.LoadModel(utils::getCurrentDir() + "/resources/models/backpack.obj");
   program.LoadTransformation([this](GLuint program)
   {
      glm::mat4 model = glm::mat4(1);
...
void Application::AddBoxes()
{
   Textures textures;
   textures.emplace_back(utils::ImageLoader().Load(utils::getCurrentDir() + "/resources/textures/texture1.jpg"));
   textures.emplace_back(utils::ImageLoader().Load(utils::getCurrentDir() + "/resources/textures/container2.png"));

   object::Object o1( std::make_unique<vertice::Position>(vertices::cube1, 3)
                    , std::make_unique<vertice::Normal>(vertices::cube1_normals, 3)
                    , std::make_unique<vertice::Texture>(vertices::cube1_tex_coords, 2));

   m_models.emplace_back(std::make_pair<Models::value_type::first_type, Models::value_type::second_type>(std::move(o1), std::move(textures)));

   for ( int i = 0; i < 5; ++i )
   {
      auto& program = AddProgram(m_shaders.object, m_models.back().second);
      program.LoadObject(m_models.back().first);
      ...
      program.LoadTransformation([this, position](GLuint program)
      {
         glm::mat4 model = glm::mat4(1);
         model = glm::translate(model, position);
         model = glm::rotate(model, (float)glfwGetTime()/2 + glm::radians(45.0f), glm::vec3(15.0, 15.0, 15.0));
         glUniformMatrix4fv(glGetUniformLocation(program, "model"), 1, GL_FALSE, glm::value_ptr(model));
...
utils::opengl::Program& Application::AddProgram(
...
void Application::SceneTransformation(GLuint program) const
{
   // camera position
